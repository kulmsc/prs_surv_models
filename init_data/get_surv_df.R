...
date_attend <- read.csv("~/athena/why_wrong/batch_indivs/date_attend.csv", stringsAsFactors=F, header=T)
...
start_date <- start_date[surv_df$time > 0]
...
#NOW USING DATE ATTEND INSTEAD OF PREVIOUS START DATE !!!!!!!!!!!!!!!!!!
...
start_date <- start_date[end_date > date_attend[,2]]
end_date <- end_date[end_date > date_attend[,2]]


surv_df$time <- as.numeric(as.Date(end_date) - as.Date(start_date))
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
birthday <- read.csv("~/athena/why_wrong/batch_indivs/date_of_birth.csv", stringsAsFactors=F, header=T)
