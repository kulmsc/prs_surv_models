...
for(i in 1:length(u_age)){
  ...
  rr_fit_subset_base[[i]] <- predict(rr_model_base, newdata = sub_test_df, cause = "diagnosis", times = seq(365, max(test_surv_df$time), 365))

  rr_model_score <- CSC(formula = as.formula(paste0("Hist(time, event_type) ~ ", score_covars)), data = sub_train_df)
  rr_fit_subset_score[[i]] <- predict(rr_model_score, newdata = sub_test_df[i,], cause = "diagnosis", times = seq(365, max(test_surv_df$time), 365))
  rr_subset_coefs[[i]] <- summary(rr_model_score$models[[3]])$coefficients[nrow(summary(rr_model_score$models[[3]])$coefficients),]

  rr_subset_eids[[i]] <- sub_test_df$eid
}
...
ms_fit_base <- get_survfit(ms_model_base, test_surv_df, seq(365, max(test_surv_df$time), 365))
ms_fit_base <- t(do.call("cbind", ms_fit_base)) #rows are the individuals, cols are the time
ms_fit_base <- list("time" = ms_fit_base[1,], "risk" = ms_fit_base[ms_fit_base[,1] != ms_fit_base[1,1],])
...
ms_fit_score <- get_survfit(ms_model_score, test_surv_df, seq(365, max(test_surv_df$time), 365))
ms_fit_score <- t(do.call("cbind", ms_fit_score)) #now has rows of time, cols of people
ms_fit_score <- list("time" = ms_fit_score[1,], "risk" = ms_fit_score[ms_fit_score[,1] != ms_fit_score[1,1],], "coef" = ms_model_coef)
...
ms_fit_subset_base <- get_survfit(ms_model_base, test_surv_df, (60:80)*365)
ms_fit_subset_base <- t(do.call("cbind", ms_fit_subset_base))
ms_fit_subset_base <- list("time" = ms_fit_subset_base[1,], "risk" = ms_fit_subset_base[ms_fit_subset_base[,1] != ms_fit_subset_base[1,1],])
...
ms_model_coef <- summary(ms_model_score)$coef[nrow(summary(ms_model_score)$coef),]
ms_fit_subset_score <- get_survfit(ms_model_score, test_surv_df, (60:80)*365)
ms_fit_subset_score <- t(do.call("cbind", ms_fit_subset_score))
ms_fit_subset_score <- list("time" = ms_fit_subset_score[1,], "risk" = ms_fit_subset_score[ms_fit_subset_score[,1] != ms_fit_subset_score[1,1],], "coef" = ms_model_coef)
...
fg_fit_base <- get_survfit(fg_model_base, test_surv_df, seq(365, max(test_surv_df$time), 365), "fg")
fg_fit_base <- t(do.call("cbind", fg_fit_base)) #now has rows of time, cols of people
fg_fit_base <- list("time" = fg_fit_base[1,], "risk" = fg_fit_base[fg_fit_base[,1] != fg_fit_base[1,1],])
...
fg_fit_score <- get_survfit(fg_model_score, test_surv_df, seq(365, max(test_surv_df$time), 365), "fg")
fg_fit_score <- t(do.call("cbind", fg_fit_score)) #now has rows of time, cols of people
fg_fit_score <- list("time" = fg_fit_score[1,], "risk" = fg_fit_score[fg_fit_score[,1] != fg_fit_score[1,1],], "coef" = fg_fit_coef)
...
fg_sub_fit_base <- get_survfit(fg_sub_model_base, test_surv_df, (60:80)*365, "fg")
fg_sub_fit_base <- t(do.call("cbind", fg_sub_fit_base)) #now has rows of time, cols of people
fg_sub_fit_base <- list("time" = fg_sub_fit_base[1,], "risk" = fg_sub_fit_base[fg_sub_fit_base[,1] != fg_sub_fit_base[1,1],])
...
fg_sub_fit_score <- get_survfit(fg_sub_model_score, test_surv_df, (60:80)*365, "fg")
fg_sub_fit_score <- t(do.call("cbind", fg_sub_fit_score)) #now has rows of time, cols of people
fg_sub_fit_score <- list("time" = fg_sub_fit_score[1,], "risk" = fg_sub_fit_score[fg_sub_fit_score[,1] != fg_sub_fit_score[1,1],], "coef" = fg_sub_fit_coef)


all_fg_res <- list("base" = fg_fit_base, "score" = fg_fit_score, "sub_base" = fg_fit_subset_base, "sub_score" = fg_fit_subset_score)
